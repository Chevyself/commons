...
public class Maps {
  ...
  /**
   * Creates a map builder. Note that this uses {@link #singleton(Object, Object)} to initialize the map
   * therefore the map is a {@link HashMap}
   *
  ...
  /**
   * Create a map from strings separating the key and the value using a dot '.'. If there's no
   * dot it will be ignored and not added. This uses the method {@link #fromStringArray(String, String...)}
   *
  ...
  /**
   * Create a map from an array of strings, each string will be separated to a key and a value with the parameter separator as follows:
   *
   * "[key][separator][value]"
   * @see #fromStringArray(String...)
   *
   * If the string does not contain the separator it will be ignored and note added to the map
   *
   ...
   */
  public static Map<String, String> fromStringArray(
  ...
  /**
   * Splits the string into an array of strings to use in {@link #fromStringArray(String...)} the regex that splits the
   * strings is just ","
   *
